...
public class matrix_mult_2 { 
	public static void main(String[] args) throws Exception { 
		...
		Job job = Job.getInstance(conf, "cosine sim."); 
		job.setJarByClass(matrix_mult_2.class); 
...
class IntSumReducer extends Reducer<Text,Text,Text,Text> { 
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{ 
		...
		double term2=0;
		for(int i=0;i<n;i++){
			term1+=Math.pow((double)(arrA[i]),2);
			term2+=Math.pow((double)(arrB[i]),2);	
			sum+=(double)(arrA[i]*arrB[i]);
		}
		if(sum==0.)return;
		sum=sum/(Math.sqrt(term1)*Math.sqrt(term2));
		String r=key.toString().split(",")[0];
		String c=key.toString().split(",")[1];
		context.write(null,new Text(r+"\t"+c+"\t"+Double.toString(sum))); 
	}
}
